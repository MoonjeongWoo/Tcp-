...
public class UDPReceiver {
	public static void main(String[] args) {
		...
		try{
			...
			// 포트 5000번에 소켓을 생성하였다.
			System.out.println("UDPReceiver 데이터그램소켓 생성");
			...
			// 클라이언트가 서버에게 보낸 메세지를 출력한다.
			
			String sendMessage = "너 여기는 Sender~라고 보냈지^^";
			// 서버가 클라이언트에게 메세지를 보낸다.
			
			byte[] sendByte = sendMessage.getBytes();
			// 메세지를 보낼때도 byte배열로 만들어야 하기 때문에 String값을 byte배열로 변환한다.
